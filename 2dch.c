...
/**
    crea la cobertura convexa.

    @parameter coord**  V   conjunto de posiciones X del conjunto de puntos
    @parameter int      n   cantidad de puntos a ser considerados en el cálculo
    @parameter function cmp función para comparar y establecer el criterio de 
                            covertura convexa.
*/
int make_chain(coord** V, int n, int (*cmp)(const void*, const void*)) {
    ...
    // se orden el arreglo V con el criterio dado por la función cmp
    qsort(V, n, sizeof(coord*), cmp);

    // evalúa cada 3 puntos. primero 2,1,0 -> ...
    for (i=2; i<n; i++) {
        for (j=s; j>=1 && ccw(V, i, j, j-1); j--){}
        s = j+1;
        // se hace un swap entre la posición s e i.
        t = V[s]; 
        V[s] = V[i]; 
        V[i] = t;
    }
...
/**
    Crea la covertura convexa de todo el conjunto de puntos, primero de la
    parte inferior y luego superior.

    @parameter coord P Arreglo con las posiciones en x, el orden es el dado al
                       ingreso de ellos (los define el input).
    @parameter int   n Cantidad de elementos a considerar del arreglo P. 

    @return cantidad de elementos que forman parte de la covertura convexa.
*/
